...
export class CargoProvider {
  ...
  async getCargo(id) {
    ...
    return await this.getFlight(cargo);
  }

  async getFlight(cargo) {
    ...
    let flight = flightList.find(item => item.id == cargo.flight.split("#")[1]);
    if (cargo)
      cargo.flightObj = flight;
  ...
  async sendTransaction(cargoId){
    ...
    let res:any = await this.http.post(
      `${this.setting.ENDPOINT}/blockchain/user/${this.userprovider.user.id}/api/org.airline.airChain.CollectCargoFromWarehouse`, data,
      {withCredentials: true}
    ).toPromise();
